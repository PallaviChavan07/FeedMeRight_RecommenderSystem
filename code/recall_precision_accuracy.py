...
def cb_evaluate_model_for_user(user_id, users_cb_recs_df, k=5):
    if users_cb_recs_df is None:
        return {'p_recall': 0, 'a_recall': 0, 'user_top_k_recos_count': 0, 'user_interated_relevant_count': 0, 'k': k, 'p_precision':0, 'p_accuracy':0}
    ...
    n_rec_k = len(user_top_k_recos.loc[user_top_k_recos['cb_score'] >= 0.5])
    # Precision@K: Proportion of recommended items that are relevant
    p_precision = n_rel_and_rec_k / n_rec_k if n_rec_k != 0 else 1
    p_accuracy = (n_rel_and_rec_k + n_irrel_and_rec_k) / k
...
def cb_recommendatons(cb_objname):
    ...
    pr_detailed_results_df = pd.DataFrame(users_metrics).sort_values('p_precision', ascending=False)
    p_global_precision = pr_detailed_results_df['p_precision'].sum() / len(pr_detailed_results_df['p_precision'])

    pac_detailed_results_df = pd.DataFrame(users_metrics).sort_values('p_precision', ascending=False)
    p_global_accuracy = pac_detailed_results_df['p_accuracy'].sum() / len(pac_detailed_results_df['p_accuracy'])

    global_metrics = {'modelName': cb_objname.get_model_name(), 'p_global_recall': p_global_recall,
                      'a_global_recall': a_global_recall, 'p_global_precision' : p_global_precision,'p_global_accuracy':p_global_accuracy }
