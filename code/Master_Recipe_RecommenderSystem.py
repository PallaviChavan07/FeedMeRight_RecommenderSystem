...
cii.ComputeCollaborativeFiltering_Item_Item(recipe_df, train_rating_df, pd, benchmark)
cuu.ComputeCollaborativeFiltering_User_User(recipe_df, train_rating_df, pd, benchmark)
...
results = pd.DataFrame.from_records(benchmark, exclude=['MSE', 'FCP'], columns=['RMSE', 'MAE', 'MSE', 'FCP', 'PrecisionAt10', 'RecallAt10'],
                                    index=['KNNBasic_Item_Item', 'KNNBasic_User_User', 'SVD', 'SVD++', 'SlopeOne', 'CoClustering'])
print(results)
